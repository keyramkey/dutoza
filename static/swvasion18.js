/* Kijiji Tanzania / JAMII TANZANIA - Service Worker (v13)
   Offline page always shows when network fails.
   Bump CACHE_VERSION when you change icons/logo/important assets.
*/

const CACHE_VERSION = 'jamii-v18';
...
self.addEventListener('fetch', (event) => {
  ...
  // A real page load (clicking a link, typing a URL) vs. a background
  // fetch/XHR call — computed early so it can guide the next check too.
  const isNavigate =
    req.mode === 'navigate' ||
    (req.headers.get('accept') || '').includes('text/html');

  // Realtime / API background calls → network only, let them fail naturally.
  // IMPORTANT: this must NOT catch real page navigations — /inbox, /chat/...,
  // /kijiji/..., /status/... are actual pages people tap into, and those
  // still need our designed offline page (or a cached copy) when offline.
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          // 1. The exact page the user asked for (if previously cached) —
          //    this is what makes "Rudi Home" / back-navigation actually work
          // 2. /home
          // 3. /
          // 4. Designed offline page
          // 5. Emergency HTML
          return (
            (await caches.match(req)) ||
